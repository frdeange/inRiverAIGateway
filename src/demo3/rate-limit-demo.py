...
def send_request(client, model, index, customer_name):
    try:
        raw = client.chat.completions.with_raw_response.create(
            model=model,
            messages=[{"role": "user", "content": f"Request {index} from {customer_name}. Reply in one word."}],
            max_completion_tokens=20,
        )
        response = raw.parse()
        remaining = raw.headers.get("x-remaining-calls", "?")
        region = raw.headers.get("x-ms-region", "?")
        return {"status": 200, "remaining_calls": remaining, "region": region}
    except (RateLimitError, APIStatusError) as exc:
        status = 429 if isinstance(exc, RateLimitError) else int(exc.status_code)
        return {"status": status, "remaining_calls": "0", "region": "N/A"}
    except Exception:
        return {"status": 0, "remaining_calls": "?", "region": "N/A"}
...
def print_result(name, i, result):
    ...
    try:
        ...
        pass
    print(f"   [{name}] #{i:02d} → {label} | calls-left: {remaining} | 🌍 {result['region']}")


def run_customer(name, client, model, num_requests, delay=0.5):
    results = []
...
def run_customer_parallel(name, client, model, num_requests):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_requests) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            i = futures[future]
            indexed_results.append((i, future.result()))
...
def main() -> int:
    ...
    endpoint = gateway_base.rsplit("/", 1)[0] + "/noisyneighbor"
    ...
    print("🚦 Rate Limiting Demo (calls per minute)")
    print(f"🎯 Endpoint: {endpoint}")
    print(f"📊 Limit: 3 calls/min per customer")
    print("=" * 65)

    # Phase 1: Customer A — 3 requests, should all pass
    print(f"\n📋 Phase 1: {name_a} (3 requests, within limit)")
    print("-" * 65)
    results_a1 = run_customer(name_a, client_a, model, 3)
    ok_a1 = sum(1 for r in results_a1 if r["status"] == 200)
    throttled_a1 = sum(1 for r in results_a1 if r["status"] == 429)
    print(f"\n   ✅ {ok_a1}/3 OK | 🚫 {throttled_a1} throttled\n")

    # Phase 2: Customer B — 12 parallel requests, most should be throttled
    print(f"📋 Phase 2: {name_b} (12 requests, ALL PARALLEL)")
    print("-" * 65)
    ...
    print(f"\n   ✅ {ok_b}/12 OK | 🚫 {throttled_b} throttled\n")

    # Phase 3: Customer A — should still work
    print(f"📋 Phase 3: {name_a} again (3 requests) — unaffected?")
    print("-" * 65)
    results_a2 = run_customer(name_a, client_a, model, 3)
    ...
    print("📊 RESULTS")
    print("=" * 65)
    print(f"   {name_a} (Phase 1): ✅ {ok_a1}/3 OK | 🚫 {throttled_a1} throttled")
    print(f"   {name_b} (flood):   ✅ {ok_b}/12 OK | 🚫 {throttled_b} throttled")
    print(f"   {name_a} (Phase 3): ✅ {ok_a2}/3 OK | 🚫 {throttled_a2} throttled")
    print()
    print("   📖 429 = rate limit exceeded (resets every minute)")
    print("   🔑 Each customer has an INDEPENDENT call counter")
    return 0
